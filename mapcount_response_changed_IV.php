...
foreach ($fqarray as $fqfile){
	...
	echo $fqfile;
        echo " is the fastq file with one !! \n \n";
...
$howManyFiles = 0;
$pathToDataStoreJobDir = "";

$fqPathsJsonCommand = "";

foreach($fqarray as $fqoriginal) {
	
	echo $fqoriginal . " is the original selection !! \n \n";
	
	// Generated by curl-to-PHP: http://incarnate.github.io/curl-to-php/
	if($howManyFiles == 0)
	{
		$pathToDataStoreJobDir = "temp_output/" . $fqoriginal[0] . "_and_others_in_job";
		echo $pathToDataStoreJobDir;
		echo " is the path !!! \n \n";
		$ch = curl_init();
		...
		curl_setopt($ch, CURLOPT_URL, "https://agave.iplantc.org/files/v2/media/$user?pretty=true");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, "action=mkdir&path=$pathToDataStoreJobDir");
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
	...
	echo $fqdoublestranded;
	echo "is the double stranded split \n";

	# don't like this but it needs to be done for clarity below
	$fq = $fqdoublestranded[0];
	echo $fq . " is the fq at 0 !!! \n \n";

	////okay, now move the file into the job directory
	...
	if (strlen($isDup) != 0)
	{
		...
		if (count($fqdoublestranded)==2) {
			echo "we have double stranded now !!";
			...
			$fqpath = "$user/fqfiles/$fqdoublestranded[0]";
			$fqpath2strand = "$user/fqfiles/$fqdoublestranded[1]";
			
			$fqPathAgave = "\"" . "agave://data.iplantcollaborative.org/" . $user . "/fqfiles/" . $fqdoublestranded[0] . "\"";
			$fqpath2strandAgave = "\"" . "agave://data.iplantcollaborative.org/" . $user . "/fqfiles/" . $fqdoublestranded[1] . "\"";
			
			$fqPathsJsonCommand .= "\"" . $fqPathAgave . "\"," . "\"" . $fqpath2strandAgave . "\"";
...
/*
			...
			echo $pathToDataStoreJobDir . " is where move !! \n";
    			//curl_setopt($ch, CURLOPT_URL, "https://agave.iplantc.org/files/v2/media/$user/Transcriptome_Delaware/744_16_CCGTCC_L002_R1_001.fastq.gz?pretty=true");
			curl_setopt($ch, CURLOPT_URL, "https://agave.iplantc.org/files/v2/media/$user/fqfiles/$fqdoublestranded[0]?pretty=true");
    			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    			curl_setopt($ch, CURLOPT_POSTFIELDS, "action=move&path=$user/$pathToDataStoreJobDir/$fqdoublestranded[0]");
    			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
			...
			echo $result . "is the move result !!! \n \n";
    			if (curl_errno($ch)) {
        			echo 'Error:' . curl_error($ch);
    			}
    			curl_close ($ch);
			...
			//Now for number two:
			 $ch = curl_init();
                        ...
                        curl_setopt($ch, CURLOPT_URL, "https://agave.iplantc.org/files/v2/media/$user/fqfiles/$fqdoublestranded[1]?pretty=true");
                        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                        curl_setopt($ch, CURLOPT_POSTFIELDS, "action=move&path=allenhub/$pathToDataStoreJobDir/$fqdoublestranded[1]");
                        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
...
*/
		...
		#do the move path here, now, too
		
		else{
			$fqpath = "$subdirectories/uploads" . "/" . $_SESSION['user_name'] . "/$fq";
			$fqPathAgave .= "\"" . "agave://data.iplantcollaborative.org/" . $user . "/fqfiles/" . $fq ."\"" . ",";

                        $fqPathsJsonCommand .= "\"" . $fqPathAgave . "\",";


			echo "$fq is fq and we have single stranded !!";
			...
			/*
				 //move both of them
                	        ...
                	        echo $pathToDataStoreJobDir . " is where move !! \n";
                        	//curl_setopt($ch, CURLOPT_URL, "https://agave.iplantc.org/files/v2/media/$user/Transcriptome_Delaware/744_16_CCGTCC_L002_R1_001.fastq.gz?pretty=true");
	                        curl_setopt($ch, CURLOPT_URL, "https://agave.iplantc.org/files/v2/media/$user/fqfiles/$fq?pretty=true");
	                        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	                        curl_setopt($ch, CURLOPT_POSTFIELDS, "action=move&path=$user/$pathToDataStoreJobDir/$fq");
	                        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
				...
				echo $result . " is the single move curl command !!";
		...
		echo "$gid2 is the gid2 !!!\n \n";
		echo strlen($gid2) . " is the string length of gid2 !!!";
		...
		if(strlen($gid2) != 0)
		{
		  ...
		  ####If there is a file, we are going to need to pull it 
		  $jsonIrod = $gid2;
		}
		else
		{
		...
		//determine if we need to pull or not
		echo $jsonIrod . " is json Irod !!!";
	...
	echo $howManyFiles . " is how many files !!!";

}


echo $fqPathsJsonCommand;
echo "is the fqpathsJsonCommand";
...
$ch = curl_init("https://agave.iplantc.org:443/files/v2/listings/$user/coge_data");
...
//determine if the selected genome is already there
$genomesFromDataStore = array();
...
for($i=0; $i<count($obj["result"]); $i++)
{
        ...
        array_push($genomesFromDataStore, $file);

}

echo "$gid is the gid !!";

//If it is already there then we set 
if (in_array($gid, $genomesFromDataStore) || (strlen($gid2) != 0)){
...
else
{
	...
	###Path to genome 
	$pathToGenome = $user . "/genome_directory/" . $gid;

        curl_setopt($ch, CURLOPT_URL, "https://agave.iplantc.org/files/v2/media/$user?pretty=true");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "action=mkdir&path=$pathToGenome");
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
        ...
        if (curl_errno($ch)) {
          ...
          curl_close($ch);
          echo $result . "is the result from mkdir !! \n";
         
	###Also change permissions so that we can bring over the file from CoGe
	// Generated by curl-to-PHP: http://incarnate.github.io/curl-to-php/
	$ch = curl_init();

	curl_setopt($ch, CURLOPT_URL, "https://agave.iplantc.org/files/v2/pems/$pathToGenome?pretty=true");
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
echo json_encode($genomesFromDataStore) . " are from the data store !! \n \n";


$fqPathAgave = rtrim($fqPathAgave, ",");

###Json Base ###
$jsonBase = "{
    \"jobName\": \"allenhub-raven-kallisto-test-0.1.19\",
    \"softwareName\": \"allenhub-raven-kallisto-test-0.1.19\", 
    \"requestedTime\": \"01:00:00\", 
    \"archive\": \"true\", 
    \"archivePath\": \"\",  
    \"inputs\": { 
        \"inputFq\": [$fqPathAgave], 
        \"genome\":\"agave://data.iplantcollaborative.org/$user/genome_directory/$gid\", 
	\"jsonIrod\": \"$jsonIrod\"
    },
    \"parameters\":{ 
...
echo $jsonBase;
echo "is the json base !!!";

###place to put the json object !!
$jsonFilePath = "$bashdir/run_" . $mytimeid. ".mapcount.json";
file_put_contents($jsonFilePath, $jsonBase);

$theStringCommand = "curl -sk -H \"Authorization: Bearer $token\" -X POST -F \"fileToUpload=@" . $commandsForUserFile . "\" 'https://agave.iplantc.org/jobs/v2/?pretty=true'";
...
//$theCommandToRun = python ".$subdirectories."/get_genome.py ".$_SESSION['access_token']. " " .$_SESSION['user_name']." ".$gid." > /dev/null;
	...
	file_put_contents($commandsForUserFile, $jsonBase);
	echo $commandsForUserFile . " is the commands for user file !!! \n";
...
if(strlen($gid2) != 0)
{
//	$theCommandToRun = python ".$subdirectories."/get_genome.py ".$_SESSION['access_token']. " " .$_SESSION['user_name']." ".$gid." ".$jsonIrod." "$jsonFilePath" > /dev/null;
}


//In this case, we don't need to pull the genome
if(strlen($gid2) != 0)
{
        $theCommandToRun = "python " .  "$subdirectories/script_to_get_the_genome_from_CoGeII.py  ".$_SESSION['access_token']. " " .$_SESSION['user_name']." ". "present" . " " . $jsonFilePath . " " . $bashfile .  " > /dev/null";
	echo $theCommandToRun . " is the command to run !!! \n \n";
...
echo $fqPathsJsonCommand  . " is the json command and stuff !! \n \n";
